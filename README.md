# Hi, I'm Ashish Rawat 👋
### Data Analyst | Power BI | SQL | Python | Excel

🎓 MCA (AI & Data Science) — Graphic Era Hill University  
📍 Dehradun, India | 📧 ashish9105335275@gmail.com  
💼 Open to Data Analyst roles | Immediate Joiner

---

## 🛠️ Skills
- **BI & Visualization:** Power BI, DAX, KPI Dashboards
- **Data:** SQL, Excel (Pivot Tables, Power Query, VLOOKUP)
- **Programming:** Python (Pandas, NumPy, OpenCV)
- **Tools:** GitHub, CRM Systems, Canva

---

## 📊 Projects
| Project | Tools | Highlights |
|---|---|---|
| Sales Dashboard | Power BI | 10K+ records, 60% reporting effort reduced |
| HR Analytics Dashboard | Power BI | 2K+ employee records, attrition tracking |
| Face Recognition Attendance | Python, OpenCV | 50+ users, 80% time saved |
| Contact Management System | C, GTK+ | 1000+ records, CRUD operations |

---

## 📈 GitHub Stats
https://github-readme-stats-one-tau-82.vercel.app/api?username=Ashish910533&show_icons=true&theme=default

https://streak-stats.demolab.com?user=Ashish910533&theme=default
![Followers](https://img.shields.io/github/followers/Ashish910533?style=social)
![Stars](https://img.shields.io/github/stars/Ashish910533?style=social)
